...
| Name | Source | Version |
|------|--------|---------|
| <a name="module_aws_auth"></a> [aws\_auth](#module\_aws\_auth) | terraform-aws-modules/eks/aws//modules/aws-auth | 20.23.0 |
| <a name="module_eks"></a> [eks](#module\_eks) | terraform-aws-modules/eks/aws | 20.23.0 |
| <a name="module_eks_blueprints_addons"></a> [eks\_blueprints\_addons](#module\_eks\_blueprints\_addons) | aws-ia/eks-blueprints-addons/aws | 1.1.1 |
...
| <a name="input_aws_auth_users"></a> [aws\_auth\_users](#input\_aws\_auth\_users) | List of IAM roles to add to the aws-auth configmap | <pre>list(object({<br>    userarn  = string<br>    username = string<br>  }))</pre> | `[]` | no |
...
| <a name="input_cluster_node_groups"></a> [cluster\_node\_groups](#input\_cluster\_node\_groups) | Configuration for the managed node groups in the EKS cluster. This should be a map with keys for each node group and values that are maps with the following keys. | `any` | n/a | yes |
...
| <a name="input_eks_addons"></a> [eks\_addons](#input\_eks\_addons) | Map of EKS add-on configurations to enable for the cluster. Add-on name can be the map keys or set with `name` | `any` | <pre>{<br>  "aws-ebs-csi-driver": {<br>    "most_recent": true<br>  },<br>  "coredns": {<br>    "most_recent": true<br>  },<br>  "kube-proxy": {<br>    "most_recent": true<br>  },<br>  "vpc-cni": {<br>    "most_recent": true<br>  }<br>}</pre> | no |
...
| <a name="input_enable_cluster_autoscaler"></a> [enable\_cluster\_autoscaler](#input\_enable\_cluster\_autoscaler) | Enable Cluster Autoscaler controller add-on | `bool` | `true` | no |
...
| <a name="input_private_subnets"></a> [private\_subnets](#input\_private\_subnets) | A list of private subnet IDs | `list(string)` | `[]` | no |
| <a name="input_public_subnets"></a> [public\_subnets](#input\_public\_subnets) | A list of public subnet IDs | `list(string)` | `[]` | no |
...
| Name | Description |
|------|-------------|
| <a name="output_cloudflare_egress_ipv4"></a> [cloudflare\_egress\_ipv4](#output\_cloudflare\_egress\_ipv4) | Cloudflare ZT static egress ipv4 addresses |
